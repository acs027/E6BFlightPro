...
export function LinkedinIcon(props: { size?: number, color?: string, insideColor?: string, hoverColor?: string  }) {
  const [hovered, setHovered] = useState(false); 
  const size = hovered ? props.size * 1.2 || 24 * 1.2 : props.size || 24;
  const hoverColor = props.hoverColor || '#ff6347';
  const theme = useColorScheme();
  const color = hovered ? hoverColor : props.color || theme === 'dark' ? '#fff' : '#000'; 
  return (
    <Svg
      width={size}
      height={size}
